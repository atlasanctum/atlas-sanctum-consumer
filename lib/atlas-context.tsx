...
type AtlasContextValue = {
  ready: boolean; onboarded: boolean; profile: AtlasProfile; actions: ActionItem[]; aiOpen: boolean; lensOpen: boolean; aiQuery: string; aiAnswer: DecisionRecommendation | null; lensResult: boolean;
  ...
  toggleAction: (id: string) => void; addAction: (title: string, detail?: string) => void; openAI: (seed?: string) => void; closeAI: () => void; setAIQuery: (value: string) => void; submitAI: () => void; openLens: () => void; closeLens: () => void; showLensResult: () => void; resetLens: () => void;
};
...
export function AtlasProvider({ children }: { children: ReactNode }) {
  const [ready, setReady] = useState(false); const [onboarded, setOnboarded] = useState(false); const [profile, setProfile] = useState<AtlasProfile>(defaultProfile); const [actions, setActions] = useState<ActionItem[]>(starterActions); const [aiOpen, setAiOpen] = useState(false); const [lensOpen, setLensOpen] = useState(false); const [aiQuery, setAiQuery] = useState(""); const [aiAnswer, setAiAnswer] = useState<DecisionRecommendation | null>(null); const [lensResult, setLensResult] = useState(false);
  ...
  const openLens = useCallback(() => { haptic.light(); setLensResult(false); setLensOpen(true); }, []); const closeLens = useCallback(() => setLensOpen(false), []); const showLensResult = useCallback(() => { haptic.success(); setLensResult(true); }, []); const resetLens = useCallback(() => setLensResult(false), []);
  const value = useMemo<AtlasContextValue>(() => ({ ready, onboarded, profile, actions, aiOpen, lensOpen, aiQuery, aiAnswer, lensResult, updateName, togglePriority, toggleValue, completeOnboarding, reopenOnboarding, toggleAction, addAction, openAI, closeAI, setAIQuery: setAiQuery, submitAI, openLens, closeLens, showLensResult, resetLens }), [actions, aiAnswer, aiOpen, aiQuery, closeAI, closeLens, completeOnboarding, lensOpen, lensResult, onboarded, openAI, openLens, profile, ready, reopenOnboarding, resetLens, showLensResult, submitAI, toggleAction, togglePriority, toggleValue, updateName, addAction]);
